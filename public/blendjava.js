...
var redirect_uri = "https://blendify-pi.vercel.app/blendhome.html";
...
function selectGenre(chosenGenreIndex) {
  genreChoice = genresForList[chosenGenreIndex];
  const button = document.getElementById("genreButton");
  button.textContent = genreChoice;
  button.style.backgroundColor = genreColors[chosenGenreIndex] || "#898989";
...
function topArtists() {
  if (artistData && artistData.length > 0) {
    artistData.forEach((artist) => {
      fetch("https://blendify-pi.vercel.app/usertopartists", {
        method: "POST",
        body: JSON.stringify({
          user_id: username,
...
function getTopArtists() {
  ...
  const endpoint = `https://blendify-pi.vercel.app/usertopartists`;
...
function callArtistRecs() {
  ...
  const search = `https://api.spotify.com/v1/search?q=genre%3A${genreChoice}&type=track&&market=USlimit=10`;
